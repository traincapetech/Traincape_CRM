...
const EmployeeDetailsModal = ({ employee, isOpen, onClose }) => {
  ...
  const renderDocumentCard = (title, document, icon) => {
    ...
    if (typeof document === 'string') {
      ...
      filename = document.originalName || document.fileName || document.name || title;
      size = document.size;
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto" aria-describedby="employee-details-description">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <User className="w-5 h-5" />
            Employee Details
        ...
        <div id="employee-details-description" className="sr-only">
          Detailed view of employee information including personal details, contact information, documents, and employment history.
        </div>
        ...
        <div className="space-y-6">
            ...
            <CardContent className="pt-6">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900">{employeeData.fullName}</h2>
                    <p className="text-gray-600">{employeeData.role?.name || employeeData.role}</p>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    <div className="text-sm text-gray-600">
                      <p className="mb-2">📋 <strong>Available Documents:</strong></p>
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        ...
                        <div className="flex items-center gap-1">
                          {employeeData.tenthMarksheet ? '✅' : '❌'} 10th Marksheet
                        </div>
                        <div className="flex items-center gap-1">
                          {employeeData.twelfthMarksheet ? '✅' : '❌'} 12th Marksheet
                        </div>
                        <div className="flex items-center gap-1">
                          {employeeData.bachelorDegree ? '✅' : '❌'} Bachelor Degree
                        </div>
                        <div className="flex items-center gap-1">
                          {employeeData.postgraduateDegree ? '✅' : '❌'} Postgraduate Degree
                        </div>
                        <div className="flex items-center gap-1">
                      ...
                      <p className="mt-3 text-blue-600 font-medium">
                        💡 Click on the "Documents" tab to view and download all documents
                      </p>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">{employeeData.phoneNumber || 'Not specified'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <MessageCircle className="w-4 h-4 text-gray-500" />
                      <span className="text-sm">{employeeData.whatsappNumber || 'Not specified'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{employeeData.currentAddress || 'Not specified'}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-700">Permanent Address:</p>
                      <p className="text-sm text-gray-600">{employeeData.permanentAddress || 'Not specified'}</p>
                    </div>
                  </CardContent>
                </Card>
              </div>
            ...
            <TabsContent value="documents" className="space-y-4">
              {console.log('Documents tab content rendering with employeeData:', employeeData)}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  {renderDocumentCard('Photograph', employeeData.photograph, <User className="w-4 h-4" />)}
                  {renderDocumentCard('Aadhar Card', employeeData.aadharCard, <FileText className="w-4 h-4" />)}
                  {renderDocumentCard('PAN Card', employeeData.panCard, <FileText className="w-4 h-4" />)}
                  {renderDocumentCard('PCC', employeeData.pcc, <FileText className="w-4 h-4" />)}
                </div>
                ...
                <div>
                  ...
                  {renderDocumentCard('10th Marksheet', employeeData.tenthMarksheet, <FileText className="w-4 h-4" />)}
                  {renderDocumentCard('12th Marksheet', employeeData.twelfthMarksheet, <FileText className="w-4 h-4" />)}
                  {renderDocumentCard('Bachelor Degree', employeeData.bachelorDegree, <FileText className="w-4 h-4" />)}
                  {renderDocumentCard('Postgraduate Degree', employeeData.postgraduateDegree, <FileText className="w-4 h-4" />)}
                </div>
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {renderDocumentCard('Resume', employeeData.resume, <FileText className="w-4 h-4" />)}
                    {renderDocumentCard('Offer Letter', employeeData.offerLetter, <FileText className="w-4 h-4" />)}
                  </div>
                </div>
              </div>
            </TabsContent>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    {employeeData.internshipDuration && (
                      <div className="flex items-center gap-2">
                        <Clock className="w-4 h-4 text-gray-500" />
                        <span className="text-sm">
                          Internship Duration: {employeeData.internshipDuration} months
                        </span>
                      </div>
                    )}
                  </CardContent>
