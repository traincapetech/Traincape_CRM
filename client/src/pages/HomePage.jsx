          ...
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center">
              {!user ? (
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center">
                  <Link
                    to={user.role === "Customer" ? "/customer" : user.role === "Sales Person" ? "/sales" : "/leads"}
                    className="glassmorphism px-6 sm:px-8 py-3 sm:py-4 rounded-full text-white font-semibold hover:bg-white hover:bg-opacity-20 transition duration-300 w-full sm:w-auto flex items-center justify-center text-sm sm:text-base"
                  >
                    <FaRocket className="mr-2 sm:mr-3" />
                    Go to Dashboard
                  ...
                  {(user.role === "Admin" || user.role === "Manager") && (
                    ...
                    >
                      ...
                      {user.role === "Admin" ? "Access Admin Dashboard" : "Access Manager Dashboard"}
                    </Link>
                  )}
                </div>
              )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 px-4 sm:px-8 lg:px-12 py-6 bg-gray-50 rounded-2xl shadow-md">
            {stats.map((stat, index) => (
              <div key={index} className="stats-card rounded-2xl p-4 sm:p-6 text-center bg-white shadow">
                ...
                <div className="text-xs sm:text-sm text-green-500 mt-1">{stat.trend}</div>
              </div>
            ))}
          </div>
        </div>
            ...
            {[
              {
                ...
                color: "blue",
                icon: <FaUsers />,
                features: ["Create and manage leads", "Assign leads to team members", "Track lead progress and status", "Generate lead reports"],
              },
              {
                role: "Sales Person",
                color: "green",
                icon: <FaChartLine />,
                features: ["Access assigned leads", "Update lead status and notes", "Schedule and track activities", "Create invoices and quotes"],
              },
              {
                role: "Manager/Admin",
                color: "purple",
                icon: <FaShieldAlt />,
                features: ["Full system access", "View all leads and sales", "Manage team and permissions", "Advanced analytics and reports"],
              },
            ].map((role, index) => (
              <div
                key={index}
                className={`bg-gradient-to-br from-${role.color}-50 to-${role.color}-100 rounded-2xl p-6 sm:p-8 border-2 border-${role.color}-200`}
              >
               ...
               {role.role === "Manager/Admin" && user && (user.role === "Admin" || user.role === "Manager") && (
                  <div className="mt-6 pt-4 border-t border-purple-200">
                    <Link
                      to="/manager"
                      className="w-full bg-purple-600 hover:bg-purple-700 text-white font-semibold py-3 px-4 rounded-lg transition duration-300 flex items-center justify-center text-sm sm:text-base"
                    >
