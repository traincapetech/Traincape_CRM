...
const LeadsPage = () => {
  ...
  useEffect(() => {
    ...
    console.log(
      `Filtered ${filtered.length} leads from ${leads.length} total leads`
    );
    setFilteredLeads(filtered);
  }, [leads, filterMonth, filterYear, showCurrentMonth]);
  ...
  const debugCurrentState = useCallback(() => {
    ...
    console.log(
      "First 3 leads in filteredLeads array:",
  ...
  const handleLeadAssign = async (leadId, newAssignee) => {
    try {
      ...
      if (response.data.success) {
        // Log the lead assignment
        try {
          await LoggingService.logLeadAssign(
            leadId,
            newAssignee,
            previousAssignee
          );
          ...
          <div className="container mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-center">
              <div>
                ...
                <p className="text-blue-100 text-sm md:text-base">
                  ...
                  {showCurrentMonth
                    ? "Current Month"
                    : `${
                        months.find((m) => m.value === parseInt(filterMonth))
                          ?.label
                      } ${filterYear}`}
                </p>
              </div>
              ...
              <button
                type="button"
                onClick={() => setShowAddForm(true)}
                className="mt-4 md:mt-0 px-4 py-2 bg-white dark:bg-slate-900 
             border border-slate-200 dark:border-slate-700 
             hover:bg-slate-100 dark:hover:bg-slate-800 
             text-slate-800 dark:text-slate-100 
             rounded-md transition-all duration-300 ease-out 
             flex items-center font-medium shadow-sm focus:outline-none 
            ...
            <>
              ...
              <div className="bg-white dark:bg-slate-900 transition-all duration-200 ease-out rounded-xl shadow-md dark:shadow-black/25 overflow-hidden mb-8">
                ...
                <div className="p-6">
                  ...
                  <div className="mt-4 p-3 bg-blue-50 rounded-lg text-sm text-blue-800 flex items-center">
                    ...
                    <span>
                      Showing {filteredLeads.length} leads for{" "}
                      {showCurrentMonth
                        ? "current month"
                        : `${
                            months.find(
                              (m) => m.value === parseInt(filterMonth)
                            )?.label
                          } ${filterYear}`}
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredLeads.map((lead) => (
                  <div
                    key={lead._id}
                    className="bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-700 rounded-lg shadow-md p-4 space-y-3 transform transition-transform duration-200 hover:scale-[1.02] hover:shadow-xl"
                  >
                    {/* Top row: Edit button on right, name centered */}
                    <div className="flex justify-between items-center border-b pb-2">
                      <div className="flex-1 text-center">
                        <h3 className="text-lg font-bold underline text-slate-900 dark:text-white truncate">
                          {lead.name}
                        </h3>
                      </div>
                      <button
                        onClick={() => setSelectedLead(lead)}
                        className="text-indigo-600 hover:text-white hover:bg-indigo-600 p-2 rounded-full transition"
                        title="Edit Lead"
                      >
                    ...
                    <div className="border-b pb-2 text-center">
                      ...
                      <span
                        className={`font-semibold px-2 py-1 rounded text-sm ${
                          lead.feedback === "Converted"
                            ? "text-green-700 bg-green-100 dark:bg-green-800"
                            : lead.feedback === "Not Interested"
                            ? "text-red-700 bg-red-100 dark:bg-red-800"
                            : "text-yellow-700 bg-yellow-100 dark:bg-yellow-800"
                        }`}
                      >
                        {lead.feedback || "Pending"}
                      </span>
                    ...
                    {/* Details */}
                    <div className="border border-dashed rounded-md p-3 bg-gray-50 dark:bg-slate-800 space-y-2 text-sm text-gray-800 dark:text-gray-200">
                      <DetailRow label="Course" value={lead.course} />
                      <DetailRow
                        label="Phone"
                        value={lead.phone || lead.countryCode}
                      />
                      {lead.email && (
                      ...
                      <DetailRow
                        label="Country"
                        value={lead.country || lead.countryCode}
                      />
                      <DetailRow
                        label="Date"
